...
#必要な外部ソフト:pixz、Firefox、geckodriver、wget、tmux
...
#BE鯖が無かったら勝手にインストールします(自動アプデのついでで実装)
...
#subprocessの引数の渡し方は呼び出すやつ,オプション1,オプション2…みたいな書き方しないとエラー吐いて無理って言われる
...
#2026/03/15 v20dev - 関数類の整理、端末制御周りの変更、終了コマンドの修正(ここまで実装)、パイプを用いたブラックリスト・ホワイトリストの編集、DM再起動の実装、BE鯖関連の処理スイッチ実装

#現状の状態
#tmuxでの起動に成功(SSHからの接続も確認)
#stopコマンドの送信に成功
#関数整理はとりあえずこの形で
#pid取得の関数が2個あるので結合して1つに纏めたい(挙動的に_get_session_pidを参照しないとエラー吐くのでそっちに統合予定)
...
def get_pid(switch: int):
	try:
		...
		if result.returncode == 0 and result.stdout.strip():
			return str(result.stdout.strip())	#PIDをstr型で返す
		else:
			return None	#プロセスが見つからない場合はNoneを返す
...
#鯖停止
async def stop_server(version: int) -> bool:
	global status
	global status_be
	global process_id
	...
	global pipe_flag_be
	if version == 0:	#JE
		integration.manager.check_process_alive("JE")	#死活確認してフラグ恒心
		if pipe_flag_je == True and status == 1:	#trumフラグが立ってる時はコマンド送信して停止させる
			await integration.manager.send_command(version="JE", command="stop")	#stopコマンド送信
			print("stopコマンド送信(JE)")
			await post_message("stopコマンドを送信しました(JE)")
			#終了した判定にする
			status = 0
			pipe_flag_je = False
			return True	#成功フラグ
		elif status != 1:	#既に停止してるか起動処理中の時は無視
			print("JE鯖は既に停止してるか起動処理中です")
			await post_message("JE鯖は既に停止してるか起動処理中です")
			return False	#失敗フラグ
		else:	#パイプ接続してない時はtmuxセッションを強制終了する
			try:
				subprocess.run(['tmux', 'kill-session', '-t', session_name_je], check=True)
				print("JE鯖を強制終了しました")
				await post_message("JE鯖を強制終了しました")
				#終了した判定にする
				status = 0
				pipe_flag_je = False
				return True	#成功フラグ
				...
				await post_message(f"セッション終了エラー: {e}")
				return False	#失敗フラグ
	elif version == 1:	#BE
		integration.manager.check_process_alive("BE")	#死活確認してフラグ恒心
		if pipe_flag_be == True and status_be == 1:	#trumフラグが立ってる時はコマンド送信して停止させる
			await integration.manager.send_command(version="BE", command="stop")	#stopコマンド送信
			print("stopコマンド送信(BE)")
			await post_message("stopコマンドを送信しました(BE)")
			#終了した判定にする
			status_be = 0
			pipe_flag_be = False
			return True	#成功フラグ
		elif status_be != 1:	#既に停止してるか起動処理中の時は無視
			print("BE鯖は既に停止してるか起動処理中です")
			await post_message("BE鯖は既に停止してるか起動処理中です")
			return False	#失敗フラグ
		else:	#パイプ接続してない時はtmuxセッションを強制終了する
			try:
				subprocess.run(['tmux', 'kill-session', '-t', session_name_be], check=True)
				print("BE鯖を強制終了しました")
				await post_message("BE鯖を強制終了しました")
				#終了した判定にする
				status_be = 0
				pipe_flag_be = False
				return True	#成功フラグ
...
class ProcessStatus(Enum):
	RUNNING = "running"	#生きてるの
	STOPPED = "stopped"	#止まってるの
	DEAD = "dead"	#死んでるの
...
class ManagedSession:
	...
	command: str	#起動コマンド
	pid: Optional[int] = None	#プロセスID
	status: ProcessStatus = ProcessStatus.STOPPED	#プロセス状態
	on_death_callback: Optional[Callable] = None	#プロセスが死んだときに呼び出すコールバック関数

#tmux関連
#tmux関連はよく分からないのでAI生成したやつを感覚で環境に合うように適合させてる version引数がその代表例(Discordのコマンドのオプションから直接指定出来るようにしてるつもり)
class TmuxSessionManager:
	def __init__(self):
		self.sessions: Dict[str, ManagedSession] = {}
		
	#セッション起動
	async def create_session(self, version: str, on_death_callback: Optional[Callable] = None) -> str:
		global status
		global status_be
		global process_id
		global process_id_be
		...
		try:
			...
			if version == "JE":
				...
				print(f"不明なバージョン: {repr(version)}")  #repr()で不可視文字も表示される
				return False
			#生きてるセッションがある場合は無視
			...
			subprocess.run([
				'tmux', 'new-session', '-d',
				'-s', session_name,
				'bash', '-c', command   #bashコマンド化
			], check=True)
			print(f"'{session_name}'のtmuxセッションを作成しました")
			time.sleep(0.5)  #セッション起動待ち
			#PIDを取得
			pid = self._get_session_pid(version)
			if version == "JE":
				process_id = str(pid)
				status = 1
				pipe_flag_je = True
			elif version == "BE":
				process_id_be = str(pid)
				status_be = 1
				pipe_flag_be = True
			print(f"'{session_name}'のPIDを取得しました: {pid}")
			#セッション情報を保存
			...
			await self.launch_gui_terminal(version)
			return True
		#例外処理
		#subprocess例外
		except subprocess.CalledProcessError as e:
	...
	def check_process_alive(self, version: str) -> ProcessStatus:
		...
		if session.pid:
			try:
				#kill -0で死活確認 ※pgrepだとtmuxセッションのPIDが返ってきてしまうため
				result = subprocess.run(['kill', '-0', str(session.pid)], capture_output=True)
				#生きてる時
				if result.returncode == 0:
	...
	#全セッション死活確認
	async def check_all_processes(self) -> Dict[str, ProcessStatus]:
		statuses = {}
		dead_sessions = []
		#全セッション確認
		...
		for session_name in dead_sessions:
			await self._handle_dead_session(session_name)
		#リスト出力
		return statuses
	...
	async def send_command(self, version: str, command: str) -> bool:
		...
		#コマンド送信
		try:
			subprocess.run(['tmux', 'send-keys', '-t', f'{session_name}:0.0', command, 'Enter'], check=True)
			return True
		except subprocess.CalledProcessError as e:
			print(f"コマンド送信エラー: {e}")
	...
	#セッション強制終了
	async def kill_session(self, version: str) -> bool:
		if version == "JE":
			session_name = session_name_je
		elif version == "BE":
			session_name = session_name_be
		...
		try:
			...
			await post_message(f"'{version}'鯖を強制終了しました")
			return True
		#例外処理
		except subprocess.CalledProcessError as e:
			print(f"セッション終了エラー: {e}")
...
class DiscordBotIntegration:
	...
	#JE鯖起動
	async def start_JE_server(self):
	...
	#BE鯖起動
	async def start_BE_server(self):
...
async def on_ready():
	...
	#JE
	try:
		result = integration.manager.check_process_alive("JE")
		...
		if pipe_flag_je == True:	#screen分岐
			...
			await TmuxSessionManager._handle_dead_session(integration.manager, "JE")	#セッション情報クリーンアップ
		await post_message("JE鯖が起動してないですを")
	#BE
	try:
		result = integration.manager.check_process_alive("BE")
		if result == ProcessStatus.RUNNING:
			print("生きてた")
		else:
		...
		if pipe_flag_be == True:	#screen分岐
			...
			await TmuxSessionManager._handle_dead_session(integration.manager, "BE")	#セッション情報クリーンアップ
...
async def com_start(interaction: discord.Interaction, boot: str):
	...
	if boot == "JE":
		...
		if status == 0:
			...
			#鯖起動
			print("起動命令送信")
			result = await integration.manager.create_session("JE", integration.async_on_process_death)	#セッション作成
			if result == True:
				await post_message("JE鯖の起動命令を送ったナリよ")
			elif result == False:
				await post_message("JE鯖は既に起動中ナリよ")
			else:
				await post_message(f'なんか上手いこと行かなかったみたいですよ(Tmux例外)\r\n例外詳細:{result}')
				status = 0	#死んだ扱いにする
				return
		...
		if status_be == 0:
			...
			try:
				...
				if binary_url is None:
					await interaction.followup.send("BE鯖のURLが取得できませんでした")
					return
				print(f"BE鯖URL取得完了: {binary_url}")
				binary_name: str = str(os.path.basename(urlparse(binary_url).path))	#ファイル名抽出
				#既存ファイル名取得
				...
				if not os.path.isfile(directory_be + '/' + file_name):
					...
					send_flag = True
					#実行ファイルダウンロード
					subprocess.run(["wget", "-P", str(directory_be), str(binary_url), "--no-check-certificate"])
					#展開
				...
				#アプデ
					...
					#実行ファイルダウンロード
					subprocess.run(["wget", "-P", directory_be, str(binary_url), "--no-check-certificate"])
					print("BE鯖DL完了")
					#展開
			...
			#鯖起動
			print("起動命令送信")
			result = await integration.manager.create_session("BE", integration.async_on_process_death)	#セッション作成
			if result == True:
...
async def mc_stop(interaction: discord.Interaction, target: str):
	if target == "JE":
		print("JE停止プロセス開始")
		result = await stop_server(0)
		if result == True:
			await interaction.response.send_message(f'JE鯖の停止命令を送ったナリよ')
		else:
			await interaction.response.send_message(f'JE鯖は既に停止してるナリよ')
	elif target == "BE":
		print("BE停止プロセス開始")
		result = await stop_server(1)
		if result == True:
			await interaction.response.send_message(f'BE鯖の停止命令を送ったナリよ')
		else:
			await interaction.response.send_message(f'BE鯖は既に停止してるナリよ')
	else:
		await interaction.response.send_message(f'その引数は無効っスよ')
#終了処理
