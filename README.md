...
### 注意事項
BE鯖の自動アプデですが、directory_beで指定したフォルダ内に[公式サイト](https://www.minecraft.net/ja-jp/download/server/bedrock)からDLした名前のzipファイルのファイル名でバージョン管理をしています。そのため、新規セットアップする際もzipファイルを対象ディレクトリ内に入れておいて下さい。さもないと自動で上書きされるので注意が必要です。
